 ...
 */
const Verse = ({
  slug, content, orbit, setActiveSection, activeSection,
}) => {
  ...
  useEffect(() => {
    ...
    if (inView) {
      rotateY.set(0)
      if (activeSection !== tag) setActiveSection(tag)
    } else {
      ...
      if (activeSection === tag) setActiveSection(null)
    }
  }, [inView])
    ...
    <VerseWrapper ref={ref}>
      <div css={[xw`relative -z-10`]}>
        ...
        <a
          ...
          name={slug}
          aria-label={slug}
          css={[xw`absolute invisible -top-14`]}
        />
        <motion.div
